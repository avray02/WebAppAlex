...
The repository is not initialized yet. There is no frontend framework, backend
framework, package manager, or test command configured at the time this file was
created.

When the stack is initialized, update this file with the real commands and
...
- Put it in its own folder under `apps/<app-name>/` unless the chosen framework
  requires a different convention.
- Add or update the portal metadata so the app appears from the home page.
...
No project commands are configured yet.

After the stack is created, replace this section with the real commands:
